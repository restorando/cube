cube.piece = function(board) {
  ...
  var div = document.createElement("div"),
      ...
      content    = selection.append("div").attr("class", "content"),
  ...
  if (mode == "edit") {
    ...
    var title = header.append("input")
        .attr("class", "title")
        .attr("type", "text")
        .attr("placeholder", "Title")
        .on("keyup.text", textchange)
        .on("focus.text", this.focus)
        .on("blur.text", this.blur);
  ...
  event.on("serialize.piece", serialize);
  
  function textchange() {
    if (timeout) clearTimeout(timeout);
    timeout = setTimeout(piece.edit, 750);
  }
  ...
  function resize() {
    ...
    piece.transition()
        .style("left", (boardPadding + position[0] * squareSize) + "px")
        .style("top", (boardPadding + position[1] * squareSize) + "px")
        .style("width", (size[0] * squareSize) - (2 * padding + 2 * border)  + "px")
        .style("height", (size[1] * squareSize) - (2 * padding + 2 * border) + "px");
  }

  function deserialize(x) {
    if(mode == "edit") { title.attr("value", x.title); }
    else { 
      header.html('').append("h3").text(x.title||" ");
      if(!x.title){
        titleHeight = 0;
  ...
  function serialize(x) {
    if(mode == "edit") x.title = title.property("value");
  }
  ...
  piece.innerSize = function() {
    var squareSize = board.squareSize();
    return [
      size[0] * squareSize - 2 * padding - 2 * border,
      size[1] * squareSize - 2 * padding - titleHeight - 2 * border
    ];
  };
...
function cube_piece_mouseSize() {
  var squareSize = cube_piece_dragBoard.squareSize();
  return cube_piece_dragResize ? [
    Math.max(minSize[0]||5, cube_piece_dragSize[0] + (/e$/.test(cube_piece_dragResize) ? 1 : /w$/.test(cube_piece_dragResize) ? -1 : 0) * (d3.event.pageX - cube_piece_dragOrigin[0]) / squareSize),
    Math.max(minSize[1]||3, cube_piece_dragSize[1] + (/^s/.test(cube_piece_dragResize) ? 1 : /^n/.test(cube_piece_dragResize) ? -1 : 0) * (d3.event.pageY - cube_piece_dragOrigin[1]) / squareSize)
  ] : cube_piece_dragSize;
}
