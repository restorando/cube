cube.piece.type.area = function(board) {
  ...
  function serialize(json) {
    ...
    json.metric = {};
    json.metric.query = query.property("value");
    json.metric.range = timeRange;
    json.metric.tier  = step;
  }

  function deserialize(json) {
    if (mode == "edit") {
      query.property("value", json.metric.query);
      time.select("input").property("value", json.metric.range / cube_piece_areaMultipler(json.metric.tier));
      time.select("select").property("value", json.metric.tier);
