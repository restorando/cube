...
tiers[week] = {
  ...
  floor: function(d) {
    ...
    var dd = new Date(d.getTime() - (weekDay - 1)*day);
    return new Date(dd.getFullYear(), dd.getMonth(), dd.getDate(), 0, 0, 0);
  },
  bin: function(d) {
    var weekDay = d.getDay() == 0 ? 7 : d.getDay();
    var dd = new Date(d.getTime() - (weekDay - 1)*day);
  ...
  step: function(d) {
    var weekDay = d.getDay() == 0 ? 7 : d.getDay();
    var dd = new Date(d.getTime() + (8 - weekDay)*day);
    return new Date(dd.getFullYear(), dd.getMonth(), dd.getDate(), d.getHours(), d.getMinutes(), d.getSeconds());
  },
  next: tiers[day],
...
tiers[month] = {
  ...
  ceil: tier_ceil,
  step: function(d) {
    var d2 = new Date(d.getTime() + (32 - d.getDate())*day);
    return new Date(d2.getFullYear(), d2.getMonth(), 1, 0, 0, 0);
  },
  next: tiers[day],
  size: function(d) {
    var dd = new Date(this.step(d).getTime() - 12*hour)
    return dd.getDate();
  }
};
...
Object.defineProperty(tiers, "units", {
  enumberable: false,
  value: { second: second, second10: second10, minute: minute, minute5: minute5, hour: hour, day: day, week: week, month: month }
});
