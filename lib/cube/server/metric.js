  ...
  function unary(expression, start, stop, tier, callback) {
    ...
    function find(start, stop, tier, callback) {
      ...
      function saveGroup(time, value, group) {
        util.log("Saving metric: " + JSON.stringify({expression: expression.source, time: time, tier: tier.key, value: value, group: group }));
      ...
      function save(time, value) {
        util.log("Saving metric: " + JSON.stringify({expression: expression.source, time: time, tier: tier.key, value: value }));
        callback(time, value);
        if (value) type.metrics.save({
          _id: {
            e: expression.source,
