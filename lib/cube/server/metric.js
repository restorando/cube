var util    = require("util"),
    ...
    options = require("../../../bin/evaluator-config.js")
  ...
  function unary(expression, start, stop, tier, callback) {
    ...
    function find(start, stop, tier, callback) {
      ...
      function computeFlat(start, stop) {
        if(tier.floor(start) < new Date(new Date() - options.metrics.horizons.calculation)){
          util.log("Unable to calculate metrics past horizon: " + start);
          start = tier.step(tier.floor(new Date(new Date() - options.metrics.horizons.calculation)))
        }
      ...
      function saveGroup(time, value, group) {
        ...
        util.log("Saving metric: " + JSON.stringify(metric));
        type.metrics.save();
    }
