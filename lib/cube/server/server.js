...
// Server -- a generic HTTP and websockets server
//
...
module.exports = function(options) {
  var server       = {},
      ...
      authenticate = authentication(db),
      id           = 0;
  ...
  primary.on("upgrade", function(request, socket, head) {    
    authenticate(request, function(authenticated){
      request.authentication_token = authenticated
      if(!options.server.authenticate || authenticated !== false && (authenticated.readable || authentication.writeable)){
  ...
  function connect(connection, request) {
    ...
    for (var i = -1, n = endpoints.ws.length, e; ++i < n;) {
      if ((e = endpoints.ws[i]).match(request.url)) {
        ...
        var authentication_token = request.authentication_token;

        return connection.on("message", function(request) {
          var payload = JSON.parse(request.utf8Data || request);
  ...
  server.start = function() {
    ...
    // Start the server!
    util.log("starting http server on port " + options.server.port);
    primary.listen(options.server.port);
  };
