...
var util = require("util"),
    ...
    tiers = require("./tiers"),
    types = require("./types"),
    options = require("../../../bin/collector-config.js");
...
exports.putter = function(db) {
  ...
  function endpoint(request) {
    ...
    if (isNaN(time) || time.getTime() == 0) return util.log("invalid time: " + request.time);
    ...
    db.collection(type + "_events", type_options, function(error, events) {
      if (error) {
        // Create a capped collection if requested in config
        flags = { safe: true, autoIndexId: true }
        if (options.collections.events.capped) {
          flags['capped'] = true
          flags['size'] = options.collections.events.capped
        }
        db.createCollection(type + "_events", flags, function(error, collection){
        ...
        // Create a capped collection if requested in config
        flags = { safe: true, autoIndexId: true }
        if (options.collections.metrics.capped) {
          flags['capped'] = true
          flags['size'] = options.collections.metrics.capped
        }
        db.createCollection(type + "_metrics", flags, function(error, collection){
    ...
    function save(events) {
      if(time < new Date(new Date() - options.metrics.horizons.invalidation)){
        util.log("Unable to invalidate metrics past horizon: " + time)
        return;
      }
  ...
  endpoint.flush = function() {
    ...
    for (var type in flushTypes) {
      ...
      if((times[1] - times[0]) > 1000 * 60 * 5){
        util.log("***Invalidating " + type + " metrics for interval greater than 5 minutes: " + times + "***");
      }
